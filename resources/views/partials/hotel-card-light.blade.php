<div class="col-xxl-6 col-xl-8 col-lg-10">
    <div class="destination-item style-three bgc-lighter" data-aos="fade-up" data-aos-duration="1500" data-aos-offset="50">
        <div class="image">
            <div class="ratting"><i class="fas fa-star"></i> 4.8</div>
            <img src="{{ asset('assets/images/destinations/hotel5.jpg') }}" alt="Hotel">
        </div>
        <div class="content">
            <span class="location"><i class="fal fa-map-marker-alt"></i> Makkah, Saudi Arabia</span>
            <h5><a href="{{ route('hotels.details', 1) }}">Dar Al Ghufran Hotel</a></h5>
            <ul class="list-style-one">
                <li><i class="fal fa-star"></i> <span>5 Star</span></li>
                <li><i class="fal fa-map"></i> <a href="#">Location</a></li>
            </ul>
            <div class="destination-footer" style="width: 115%;">
                <span class="price">
                    <span>$85.00</span>/night
                </span>
                <a href="#" class="read-more ml-3">Book Now <i class="fal fa-angle-right"></i></a>
            </div>
        </div>
    </div>
</div>
